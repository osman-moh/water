$(document).ready( function(){
    	...
    	ajax: {
      	...
      	language: {
		    noResults: function(){
		        ...
		        return '<button type="button" data-name="' + name + '" class="btn btn-link" id="add_new_customer"><i class="fa fa-plus-circle fa-lg" aria-hidden="true"></i>&nbsp; ' + __translate('add_name_as_new_customer', {'name': name}) +'</button>';
		    }
		},
	...
	$(document).on('click', '#add_new_customer', function(){
		...
		$('.add_new_customer_modal').find('input#name').val(name);
		$('.add_new_customer_modal').modal('show');
	});
	$("form#add_new_customer_form").submit(function(e){
    	e.preventDefault();
    }).validate({
		submitHandler: function(form) {
			var data = $(form).serialize();
			$.ajax({
				...
				success: function(result){
					if(result.success == true){
						...
						$('div.add_new_customer_modal').modal('hide');
						toastr.success(result.msg);
					} else {
						toastr.error(result.msg);
					}
	...
	$('.add_new_customer_modal').on('hidden.bs.modal', function () {
	  	$('form#add_new_customer_form')[0].reset();
	});
	...
	//Show product list.
	get_product_suggestion_list($("select#product_category").val(), $('input#location_id').val(), null);
	$("select#product_category").on("change", function(e) {
		var location_id = $('input#location_id').val();
		if(location_id != '' || location_id != undefined){
			get_product_suggestion_list($(this).val(), location_id, null);
		}
	});
	...
	$(document).on('click', 'ul.pagination_ajax a', function(e){
		...
		get_product_suggestion_list(category_id, location_id, url);
	});
...
function get_product_suggestion_list(category_id, location_id, url = null){
	if(url == null){
	...
	$.ajax({
		method: "GET",
		url: url,
		data: {category_id: category_id, location_id: location_id},
		dataType: "html",
		success: function(result){			
...
function calculate_billing_details(price_total){
	...
	var total_payable = price_total + order_tax - discount;
