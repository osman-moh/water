...
$(document).ready(function(){
					...
					columns: [
                        ...
                        createdRow: function( row, data, dataIndex ) {
                        	if($('input#is_rack_enabled').val() == 1){
                        		$( row ).find('td:eq(0)').prepend('<i class="fa fa-plus-circle text-success cursor-pointer no-print rack-details" title="' + LANG.details + '"></i>&nbsp;&nbsp;');
                        	}
                        }
			    });
	...
	$(document).on( 'change', '#auto_generate_sku', function(e){
		if($(this).is(":checked")){
			$('input#sku').prop('disabled', true)
		} else {
			$('input#sku').prop('disabled', false)
		}
	});
